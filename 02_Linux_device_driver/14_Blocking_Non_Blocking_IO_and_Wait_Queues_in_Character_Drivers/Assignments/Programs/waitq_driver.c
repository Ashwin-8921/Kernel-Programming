...
static int reader_blocked_count = 0;
static int reader_resumed_count = 0;
...
static ssize_t dev_read(struct file *file, char __user *buf, size_t len, loff_t *off)
{
    ...
    printk(KERN_INFO "[%s] Reader blocked: %d, Reader resumed: %d\n",
           DEVICE_NAME, reader_blocked_count, reader_resumed_count);
...
static void __exit waitq_exit(void)
{
    ...
    printk(KERN_INFO "[%s] unloaded. Total blocked: %d, Total resumed: %d\n",
           DEVICE_NAME, reader_blocked_count, reader_resumed_count);
}
...
MODULE_DESCRIPTION("Char driver with wait queue blocking read and reader count tracking");
